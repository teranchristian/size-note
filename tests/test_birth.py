...
def test_partial_birth_becomes_adult_only_when_definitely_eighteen():
    assert effective_growth_stage("adult", 2008, today=date(2026, 12, 31)) == "child"
    assert effective_growth_stage("child", 2008, today=date(2027, 1, 1)) == "adult"
    assert effective_growth_stage("child", 2008, 5, today=date(2026, 5, 1)) == "child"
    assert effective_growth_stage("child", 2008, 5, today=date(2026, 6, 1)) == "adult"


def test_age_aware_review_intervals_use_younger_possible_age():
    assert review_interval_days(
        "child", 2024, None, None, item_key="t-shirt", shoe_keys=SHOES, today=date(2026, 8, 30)
    ) == 90
    assert review_interval_days(
        "child", 2022, 1, 1, item_key="t-shirt", shoe_keys=SHOES, today=date(2026, 8, 30)
    ) == 120
    assert review_interval_days(
        "child", 2017, 1, 1, item_key="t-shirt", shoe_keys=SHOES, today=date(2026, 8, 30)
    ) == 180
    assert review_interval_days(
        "child", 2010, 1, 1, item_key="t-shirt", shoe_keys=SHOES, today=date(2026, 8, 30)
    ) == 270


def test_child_without_birth_uses_original_item_fallback():
    assert review_interval_days(
        "child", None, None, None, item_key="shoes", shoe_keys=SHOES, today=date(2026, 8, 30)
    ) == 90
    assert review_interval_days(
        "child", None, None, None, item_key="t-shirt", shoe_keys=SHOES, today=date(2026, 8, 30)
    ) == 180
